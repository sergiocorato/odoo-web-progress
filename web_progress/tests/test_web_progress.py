...
class WebProgressTest(common.SavepointCase):
    def check_all_progress_data_empty(self):
        ...
        self.assertFalse(
            last_report_time,
            msg="Global variable last_report_time shall be empty by now",
        )
    ...
    def setUp(self):
        ...
        self.partner_vals = {}
        for idx in range(20):
            self.partner_vals[idx] = dict(
                name="Test{}".format(idx), email="email{}@test.me".format(idx)
            )
    ...
    def _check_web_progress_iter_recordset(self, total, recur_level=0):
        ...
        """
        progress_iter = self.partner_ids[:total].with_progress(
            msg="Total {} Level {}".format(total, recur_level)
        )
        ...
        if total > 0:
            self.assertEqual(
                progress_iter[0],
                self.partner_ids[0],
                msg="Indexing shall be accessible",
            )
            self.assertEqual(
                progress_iter._name,
                self.partner_ids._name,
        ...
        for idx, partner_id in zip(range(total), progress_iter):
            self.assertEqual(
                partner_id.name,
                self.partner_vals[idx]["name"].format(idx),
                msg="Wrong name",
            )
            self.assertEqual(
                partner_id.email,
                self.partner_vals[idx]["email"].format(idx),
                msg="Wrong email",
            )
            count += 1
            if recur_level > 0:
    ...
    def test_web_progress_iter_with_web_progress_code_cancel(self):
        ...
        with self.assertRaises(
            exceptions.UserError, msg="Exception UserErro shall have been raised"
        ):
            self._check_web_progress_iter_recordset_many(0)
        self._check_web_progress_cancelled()
