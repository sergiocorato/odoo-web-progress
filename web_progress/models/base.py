...
class Base(models.AbstractModel):
    ...
    def with_progress(self, msg="", total=None, cancellable=True, log_level="info"):
        ...
        """
        return self.web_progress_iter(
            self, msg=msg, total=total, cancellable=cancellable, log_level=log_level
        )
    ...
    @api.model
    def web_progress_iter(
        self, data, msg="", total=None, cancellable=True, log_level="info"
    ):
        """
        Progress reporting generator of an ongoing operation identified by progress_code
         in context.
        ...
        return GeneratorWithLenIndexable(
            self.env["web.progress"]._report_progress(
                data, msg=msg, total=total, cancellable=cancellable, log_level=log_level
            ),
            total,
            data,
        )
    ...
    def web_progress_cancel(self, code=None):
        """
        Cancel progress of current operation or, if code given by argument, an operation
         of a given progress code
        :param code:
        """
        if code is None:
    ...
    def __iter__(self):
        ...
        if self._context.get("progress_iter"):
            ...
            return self.web_progress_iter(
                self, _("Iterating on model {}").format(self._description)
            ).__iter__()
        else:
    ...
    def _extract_records(self, fields_, data, log=lambda a: None, limit=float("inf")):
        ...
        It adds progress reporting to all standard imports and additionally makes them
         cancellable
        """
        ...
        if "progress_code" in self._context:
            ...
            return self.web_progress_iter(
                extracted,
                _("importing to {}").format(self._description.lower()),
                total=total,
                cancellable=True,
                log_level="info",
            )
    ...
    def _export_rows(self, fields, *args, _is_toplevel_call=True):
        ...
        if _is_toplevel_call and "progress_code" in self._context:
            ...
            def splittor(rs):
                ...
                """
                for idx in self.web_progress_iter(
                    range(0, len(rs), 1000),
                    _("exporting batches of 1000 lines")
            ...
            for sub in splittor(self):
                ret += super(Base, sub)._export_rows(
                    fields, _is_toplevel_call=_is_toplevel_call
                )
            return ret
        return super(Base, self)._export_rows(
            fields, _is_toplevel_call=_is_toplevel_call
        )
